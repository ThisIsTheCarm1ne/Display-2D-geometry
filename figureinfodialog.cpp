    ...
    , ui(new Ui::FigureInfoDialog)
{
    ...
    QStringList headers = {"Shape", "Size", "Rotation", "Location X", "Location Y"};
    ui->tableWidget->setHorizontalHeaderLabels(headers);

    connect(ui->tableWidget,
            &QTableWidget::itemClicked,
            this,
            &FigureInfoDialog::on_tableWidget_itemClicked);
}
...
void FigureInfoDialog::updateTable(const QList<QMap<QString, QVariant>> &figures)
{
    ...
    for (int row = 0; row < figures.size(); ++row) {
        ...
        ui->tableWidget->setItem(row, 3, new QTableWidgetItem(figure["locationX"].toString()));
        ui->tableWidget->setItem(row, 4, new QTableWidgetItem(figure["locationY"].toString()));
    }
}
